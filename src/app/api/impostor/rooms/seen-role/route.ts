...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!joinCode || !playerToken) {
      return NextResponse.json({ error: "Join code and player token are required." }, { status: 400 });
    }
    ...
    const message = error instanceof Error ? error.message : "Failed to update reveal state.";
